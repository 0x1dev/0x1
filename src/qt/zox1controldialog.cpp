...
#include "ui_zox1controldialog.h"
...
std::list<std::string> ZOx1ControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZOx1ControlDialog::listMints;

ZOx1ControlDialog::ZOx1ControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZOx1ControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZOx1ControlDialog::~ZOx1ControlDialog()
{
    delete ui;
}

void ZOx1ControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZOx1ControlDialog::updateList()
{
...
void ZOx1ControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZOx1ControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZOx1_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZOx1ControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZOx1ControlDialog::GetSelectedMints()
{
...
void ZOx1ControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
