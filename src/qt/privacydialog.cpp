                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zOX1 ought to be enough for anybody." - Bill Gates, 2017
    ui->zOX1payAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzOX1SyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzOX1->setEnabled(false);
        ui->pushButtonMintzOX1->setToolTip(tr("zOX1 is currently disabled due to maintenance."));

        ui->pushButtonSpendzOX1->setEnabled(false);
        ui->pushButtonSpendzOX1->setToolTip(tr("zOX1 is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zOX1payAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzOX1_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zOX1 is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zOX1...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zOX1 in ") + 
...
void PrivacyDialog::on_pushButtonSpendzOX1_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zOX1 is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zOX1
        sendzOX1();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zOX1
    sendzOX1();
}

void PrivacyDialog::on_pushButtonZOx1Control_clicked()
{
    ZOx1ControlDialog* zOx1Control = new ZOx1ControlDialog(this);
    zOx1Control->setModel(walletModel);
    zOx1Control->exec();
}

void PrivacyDialog::setZOx1ControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzOx1Selected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzOX1()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Ox1 Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zOX1payAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zOX1 is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " OX1 </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zOX1</b>";
    ...
    // use mints from zOx1 selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZOx1ControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZOx1ControlDialog::GetSelectedMints();
    }
    ...
    // Clear zox1 selector in case it was used
    ZOx1ControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zOx1 Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Ox1, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zOx1 Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zOX1 </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zOX1 "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zOX1 "));
    ui->labelzOX1AmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzOX1SyncStatus->setVisible(fShow);
}
