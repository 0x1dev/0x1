...
*Setup instructions for a gitian build of OX1 using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the OX1
...
Clone the git repositories for ox1 and gitian and then checkout the ox1 version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/0x1project/ox1.git
cd ox1
...
Follow the instructions in [doc/release-process.md](release-process.md) in the ox1 repository
...
To build OX1 (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the ox1 repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ox1/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/0x1project/ox1
...
For example:
```bash
URL=https://github.com/crowning-/ox1.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit ox1=${COMMIT} --url ox1=${URL} ../ox1/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ox1=${COMMIT} --url ox1=${URL} ../ox1/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ox1=${COMMIT} --url ox1=${URL} ../ox1/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ox1-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ox1-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/ox1-build.assert
```
...
[ox1/gitian.sigs](https://github.com/0x1project/gitian.sigs/) repository, or if that's not possible to create a pull
request.
